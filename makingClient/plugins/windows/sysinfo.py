...
def collect():
    data = {
        ...
        'os_release': "%s %s %s" %(platform.release(), platform.architecture()[0]),
...
class Win32Info(object):
    ...
    def get_ram_info(self):
        ...
        for item in ram_collections:
            item_data = {}
            mb = int(1024*1024)
            ram-size = int(item.Capacity)/mb
            item_data = {
                "slot": item.DeviceLocator.strip(),
                "capacity": ram_size,
                "model":item.Caption,
                "manufactory": item.Manufactor,
    ...
    def get_server_info(self):
        ...
        data['wake_up_type'] = computer_info.WakeUpIype
